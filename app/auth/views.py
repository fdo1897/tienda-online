...
def logout():
    logout_user()
    flash('Has cerrado session con exito')
    return redirect(url_for('main.index'))
...
def confirm(token):
    ...
    if current_user.confirm(token):
        flash('Tu has confirmado tu cuenta. Gracias!')
    else:
...
def change_email_request():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.password.data):
            ...
            flash('Se le ha enviado un correo electrónico con instrucciones para confirmar su nueva dirección de correo electrónico')
            return redirect(url_for('main.index'))
        else:
...
def change_email(token):
    if current_user.change_email(token):
        flash('Su dirección de correo electrónico ha sido actualizada.')
    else:
